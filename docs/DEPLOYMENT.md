...
**Read side, opt in.** Separately from the three topologies, **Settings, Calibre tab, Calibre is authoritative for owned books** (`calibre.authoritative_library_enabled`, default `false`) records operator intent for Calibre/CWA to own the metadata of books it holds. Authoritative mode is designed to read `metadata.db` read-only instead of importing those books into Bindery's catalogue. Enabling the setting today records this configuration only; live owned-library reading, matching, and owned-state integration arrive in later slices and enabling the setting does not yet change catalogue or ownership behavior. It needs `calibre.library_path` set and refuses to be enabled without one; clearing that path while it is on is refused too. Nothing about the existing behaviour changes while it is off. Design contract: [`docs/design/calibre-authoritative-library.md`](design/calibre-authoritative-library.md).
