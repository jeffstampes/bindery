...
**Read side, opt in.** Separately from the three topologies, **Settings, Calibre tab, Calibre is authoritative for owned books** (`calibre.authoritative_library_enabled`, default `false`) makes Calibre/CWA the metadata authority for its owned books. With a configured `calibre.library_path`, scheduled/manual reconciliation reads `metadata.db` without writing to it, matches external Bindery works to owned ebooks without importing them as catalogue rows, and records advisory discrepancies between the owned copy and stored external evidence for later human review. The backend audit has no review UI yet; it does not correct or tag Calibre books. Enabling requires a library path; clearing that path while the mode is on is refused. Existing behavior is unchanged while it is off. Design contract: [`docs/design/calibre-authoritative-library.md`](design/calibre-authoritative-library.md).
