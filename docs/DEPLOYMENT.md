...
**Read side, opt in.** Separately from the three topologies, **Settings, Calibre tab, Calibre is authoritative for owned books** (`calibre.authoritative_library_enabled`, default `false`) makes Calibre/CWA the metadata authority for its owned books. With a configured `calibre.library_path`, scheduled/manual reconciliation reads `metadata.db` without writing to it, matches external Bindery works to owned ebooks without importing them as catalogue rows, and records advisory discrepancies between the owned copy and stored external evidence for human review under **Activity → Calibre audit**. Enabling requires a library path; clearing that path while the mode is on is refused. Existing behavior is unchanged while it is off.

**Sole optional write-back exception.** Independently opt in with the admin setting `calibre.audit_tag_write_enabled` (default `false`) through `PUT /api/v1/setting/calibre.audit_tag_write_enabled` with `{"value":"true"}`. This requires authoritative mode enabled and a writable `calibre.library_path` mount. Only the fixed, Bindery-owned `BinderyMismatch` tag is added or removed on existing books based on current unresolved, actionable audit findings; no other Calibre metadata or tags are replaced. A failed tag write leaves audit state intact and is retried on the next recheck (see `tagError` in recheck status). The official distroless image does not need `calibredb` for this path. Turning the setting off stops all writes, including tag removal; see the [Calibre integration guide](Calibre-Integration-Wiki.md#optional-binderymismatch-tag-in-calibrecwa) for caching/concurrency caveats and the [design contract](design/calibre-authoritative-library.md) for boundaries.
