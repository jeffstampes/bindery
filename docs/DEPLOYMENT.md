...
**Read side, opt in.** Separately from the three topologies, **Settings, Calibre tab, Calibre is authoritative for owned books** (`calibre.authoritative_library_enabled`, default `false`) states that Calibre/CWA owns the metadata of the books it already holds, so Bindery reads `metadata.db` read-only instead of importing those books into its own catalogue. It needs `calibre.library_path` set and refuses to be enabled without one; clearing that path while it is on is refused too. Nothing about the behaviour above changes while it is off, and today enabling it records the choice only — owned-book matching and the metadata audit are later work. Design contract: [`docs/design/calibre-authoritative-library.md`](design/calibre-authoritative-library.md).
